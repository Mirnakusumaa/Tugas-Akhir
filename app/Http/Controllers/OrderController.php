...
class OrderController extends Controller
{
	...
	public function confirm()
	{
		...
		//auth::login($user);
		return view('layouts.confirm')->with('orders',$orders);
	}

	public function index()
	{
		//$orders = Order::where('id_user',\Auth::user()->id_user)->orderBy('id_order','desc')->first();
		//$orders = Order::get();//->orderBy('id_order','desc'); //mengambil id user
		$orders = Order::where('id_user',\Auth::user()->id_user)->orderBy('id_user','desc')->paginate(10);
		return view('layouts.status',compact('orders'));
	}
	...
	/*public function updateStatus()
	{
		...
		// $orders = Order::find(Auth::user()->id_user);
		//auth::login($user);
		return view('layouts.orderAdmin')->with('orders',$orders);
	}*/
}
